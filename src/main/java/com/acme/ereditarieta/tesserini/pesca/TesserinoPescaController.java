...
@RestController
@RequestMapping("/tesseriniPesca")
public class TesserinoPescaController {
	@Autowired
	TesserinoPescaRepository tesserinoRepository;
	...
	@PostMapping
	public ResponseEntity<String> insertTesserino(@RequestBody TesserinoPesca t) {
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> updateTesserino(@RequestBody TesserinoPesca t, 
			@PathVariable long id) {
		if (tesserinoRepository.existsById(id)) {
			Tesserino tesserinoInDB = tesserinoRepository.findById(id).get();	
			BeanUtils.copyProperties(t, tesserinoInDB);
			tesserinoRepository.save((TesserinoPesca)tesserinoInDB);
	...
	@GetMapping
	public List<TesserinoPesca> getTuttiTesserini() {
		return (List<TesserinoPesca>) tesserinoRepository.findAll();
	}
}
